...
useEffect(() => {
  const fetchTingkat = async () => {
    try {
      const res = await axios.get('http://localhost:8080/api/v1/admin/class-levels', {
        headers,
      });
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://localhost:8080/api/v1/treasurer/classes', {
        headers,
      });
...
const handleTambahKelas = async () => {
  ...
  if (formValues) {
    try {
      await axios.post('http://localhost:8080/api/v1/admin/classes', formValues, {
        headers,
      });
  ...
  if (formValues) {
    try {
      await axios.put(`http://localhost:8080/api/v1/admin/classes/${kelas.id}`, formValues, {
        headers,
      });
